...
/**
 * Use LLM to detect whether a new learning is semantically equivalent to
 * any existing learning, even if phrased differently.
 *
 * @returns {{ isDuplicate: boolean, matchedId: string|null }}
 */
async function findSemanticDuplicate(newText, existingLearnings, config) {
  if (!isLLMAvailable(config)) {
    return { isDuplicate: false, matchedId: null };
  }

  const active = existingLearnings.filter((l) => !l.archived);
  if (active.length === 0) {
    return { isDuplicate: false, matchedId: null };
  }

  // Batch: max 50 learnings to keep prompt manageable
  const batch = active.slice(0, 50);
  const listText = batch.map((l) => `${l.id} | ${l.text}`).join("\n");

  const prompt = `Detect whether a new developer preference is semantically equivalent to any existing one.

New learning: "${newText}"

Existing learnings (id | text):
${listText}

Does the new learning express the same core idea as any existing one, even if phrased differently?
Respond with JSON only:
{"duplicate": true, "matched_id": "<id>"} or {"duplicate": false}`;

  try {
    const model = config.classifier_model;
    ...
    recordCall("dedup", model, result.usage);

    const cleaned = result.text.replace(/```json\s*|```\s*/g, "").trim();
    const parsed = JSON.parse(cleaned);

    if (parsed.duplicate && parsed.matched_id) {
      return { isDuplicate: true, matchedId: parsed.matched_id };
    }
    return { isDuplicate: false, matchedId: null };
  } catch (e) {
    log(`Semantic dedup error: ${e.message}`);
    return { isDuplicate: false, matchedId: null };
  }
}
...
async function addCandidate(learning) {
  ...
  // First: try LLM-based semantic dedup
  const { isDuplicate, matchedId } = await findSemanticDuplicate(
    learning.text,
    data.learnings,
    config
  );

  // Find existing to reinforce — prefer LLM match, fall back to structural match
  let existing = null;
  if (isDuplicate && matchedId) {
    existing = data.learnings.find((l) => l.id === matchedId && !l.archived);
    if (existing) {
      log(`LLM dedup matched: "${learning.text}" → existing "${existing.text}"`);
    }
  }

  if (!existing) {
    // Structural fallback (exact text or same prefix)
    existing = data.learnings.find((l) => {
...
async function addObservation(obs) {
  ...
  // LLM-based dedup against regular (non-inferred) learnings
  const regularLearnings = data.learnings.filter((l) => !l.archived && !l.inferred);
  const { isDuplicate: isRegularDup, matchedId: regularMatchId } = await findSemanticDuplicate(
    obs.text, regularLearnings, config
  );

  if (isRegularDup && regularMatchId) {
    const existingRegular = data.learnings.find((l) => l.id === regularMatchId);
    if (existingRegular) {
      ...
      log(`Observation corroborates existing (LLM): "${existingRegular.text}" → conf ${existingRegular.confidence.toFixed(2)}`);
      saveLearnings(data);
      return;
    }
  }

  // Structural fallback for regular learnings
  ...
  // LLM-based dedup against inferred learnings
  const inferredLearnings = data.learnings.filter((l) => !l.archived && l.inferred);
  const { isDuplicate: isInferredDup, matchedId: inferredMatchId } = await findSemanticDuplicate(
    obs.text, inferredLearnings, config
  );

  if (isInferredDup && inferredMatchId) {
    const existingInferred = data.learnings.find((l) => l.id === inferredMatchId);
    if (existingInferred) {
      ...
      log(`Inferred reinforced (LLM): "${existingInferred.text}" → conf ${existingInferred.confidence.toFixed(2)}`);
      saveLearnings(data);
      return;
    }
  }

  // Structural fallback for inferred learnings
...
function getCandidates() {
  ...
  return data.learnings
    .filter((l) => !l.archived && !l.promoted && !l.inferred && l.confidence < ACTIVATION_THRESHOLD)
...
module.exports = {
  ...
  getPromotable, getCandidates, markPromoted,
