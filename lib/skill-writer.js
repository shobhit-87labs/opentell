...
function buildStatus() {
  ...
  lines.push(`Instinct — ${all.length} learnings (${active.length} active, ${candidates.length} candidates, ${promoted.length} promoted${inferredNote})\n`);
  ...
  if (inferred.length > 0) {
    ...
    lines.push("  Run: instinct observations   (to accept or reject)");
    lines.push("");
  }
  ...
  if (promotable.length > 0) {
    ...
    lines.push(`\nRun: instinct promote`);
    lines.push("");
  }

  if (all.length === 0) {
    lines.push("No learnings yet. Keep using Claude Code — Instinct will learn from your corrections.");
  }
