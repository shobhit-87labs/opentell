...
async function classifyBatch(pairs) {
  ...
  if (!isLLMAvailable(config)) {
    log("No LLM configured, skipping LLM classification");
    return pairs.map(() => ({ classification: "SKIPPED", reason: "No LLM configured" }));
  }

  const results = [];
  for (const pair of pairs) {
    try {
      const result = await classifySingle(pair, config);
      results.push(result);
    } catch (e) {
...
async function classifySingle(pair, config) {
  const model = config.classifier_model || DEFAULT_CLASSIFIER_MODEL;
  ...
  recordCall("classification", model, result.usage);
