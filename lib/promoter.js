...
const INSTINCT_HEADER = "# Instinct — Learned Preferences";
const INSTINCT_START = "<!-- instinct:start -->";
const INSTINCT_END = "<!-- instinct:end -->";
 ...
 * 
 * Inserts them in a fenced section so Instinct can update without
 * touching user-written content.
 * 
 * Returns { promoted: [...], claudeMdPath: string }
 */
function promoteToClaudeMd(projectRoot = null) {
  ...
  // Build the instinct section content
  const sectionLines = [];
  sectionLines.push(INSTINCT_HEADER);
  sectionLines.push("");
  sectionLines.push("These preferences were automatically discovered by Instinct from your corrections.");
  ...
  if (existing.includes(INSTINCT_START) && existing.includes(INSTINCT_END)) {
    // Replace existing instinct section
